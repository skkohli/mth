...
(function ($) {
  ...
  if (window._purioChatUiInit) return;
  window._purioChatUiInit = true;
  ...
  const _0x = {
    ...
    u: "aHR0cHM6Ly9wdXJldGhlbWVzLm5ldC9haS1jaGF0Ym90LWZvci13b3JkcHJlc3MvP3V0bV9zb3VyY2U9Y2hhdGJvdC13aWRnZXQmdXRtX21lZGl1bT1wb3dlcmVkLWJ5JnV0bV9jYW1wYWlnbj1icmFuZGluZw==",
  };

  const _assetBase = (function() {
    const script = document.currentScript ||
      document.querySelector('script[src*="ai-chat-search/assets/js/purio-chat-ui-utils.js"]');
    const src = script && script.src ? script.src : '';
    return src.replace(/assets\/js\/[^?#]+(?:[?#].*)?$/, '');
  })();
  const _logoUrl = _assetBase ? _assetBase + 'assets/icons/purio.svg' : 'assets/icons/purio.svg';
  ...
  function _applyBadgeStyles(badge, uniqueClass) {
    ...
    Object.keys(_criticalStyles).forEach(function(prop) {
      const value = prop === 'margin' && badge.closest('.listeo-ai-chat-wrapper.dark-mode')
        ? '-8px 0 10px 0'
        : _criticalStyles[prop];
      badge.style.setProperty(prop, value, 'important');
    });

    const link = badge.querySelector('a');
    if (link) {
      const linkColor = badge.closest('.listeo-ai-chat-wrapper.dark-mode') ? '#fff' : '#111';
      link.style.setProperty('--ai-chat-primary-color', linkColor);
      ...
      const logo = link.querySelector('.listeo-ai-chat-powered-by-logo');
      if (logo) {
        logo.style.setProperty('display', 'inline-block', 'important');
        logo.style.setProperty('position', 'relative', 'important');
        logo.style.setProperty('top', '1px', 'important');
        logo.style.setProperty('width', '16px', 'important');
        logo.style.setProperty('height', '16px', 'important');
        logo.style.setProperty('border-radius', '4px', 'important');
        logo.style.setProperty('margin', '0px 2px 0px 2px', 'important');
        logo.style.setProperty('vertical-align', '-3px', 'important');
      }

      const name = link.querySelector('.listeo-ai-chat-powered-by-name');
      if (name) {
  ...
  function _createBadge(uniqueClass) {
    ...
    logo.className = 'listeo-ai-chat-powered-by-logo';
    logo.src = _logoUrl;
    logo.alt = '';
    ...
    name.className = 'listeo-ai-chat-powered-by-name';
    name.textContent = _0x.n;
    name.style.setProperty('font-weight', '600', 'important');
  ...
  function _injectDynamicCSS(uniqueClass) {
    ...
    style.textContent = `
      ...
      .${uniqueClass} a .listeo-ai-chat-powered-by-logo {
      ...
      .${uniqueClass} a .listeo-ai-chat-powered-by-name {
        font-weight: 600 !important;
      }
      .listeo-ai-chat-wrapper.dark-mode .${uniqueClass} a {
        --ai-chat-primary-color: #fff;
        color: #fff !important;
      }
      .listeo-ai-chat-wrapper.dark-mode .${uniqueClass} {
    ...
    chatWrappers.forEach(function(chatWrapper) {
      ...
      function lockSubmitButton(reason) {
        ...
        if (chatContainer && !chatContainer.querySelector(".badge-tampering-warning")) {
          ...
          warning.textContent = "Oops! You hid the PurioChat badge";

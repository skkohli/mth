 ...
 *
 * Handles self-hosted plugin updates for Listeo Data Scraper
...
class LDS_Updater {
     ...
     */
    private $update_url = 'https://purethemes.net/license/plugins/listeo-data-scraper.json';

    /**
     * Plugin slug
     */
    private $plugin_slug = 'listeo-data-scraper';

    /**
     * Plugin file (relative path)
     */
    private $plugin_file = 'listeo-data-scraper/listeo-data-scraper.php';

    /**
     * Cache key for update check
     */
    private $cache_key = 'listeo_data_scraper_update_data';
    ...
    public function __construct() {
        $this->current_version = LDS_VERSION;

        // Allow filtering the update URL (for testing)
        $this->update_url = apply_filters('listeo_data_scraper_update_url', $this->update_url);

        // Allow filtering the check interval
        $this->cache_interval = apply_filters('listeo_data_scraper_update_check_interval', $this->cache_interval);
        ...
        add_action('wp_ajax_listeo_data_scraper_check_update_now', array($this, 'manual_update_check'));
    }
    ...
    private function fetch_update_info() {
        ...
        if (is_wp_error($response)) {
            ...
            if (get_option('lds_enable_debug_mode', false)) {
                lds_log('Update check failed: ' . $response->get_error_message(), 'UPDATER', 'ERROR');
            }
            return false;
        }
        ...
        $update_data = (object) array(
            ...
            'url' => isset($data->homepage) ? $data->homepage : 'https://purethemes.net/',
    ...
    public function plugin_row_meta($links, $file) {
        if ($file === $this->plugin_file) {
            $links[] = '<a href="https://purethemes.net/" target="_blank">' . __('View Details', 'listeo-data-scraper') . '</a>';
        }
        return $links;
    }
    ...
    public function manual_update_check() {
        check_ajax_referer('lds_ajax_nonce', 'nonce');

        if (!current_user_can('update_plugins')) {
            wp_send_json_error(array('message' => __('Insufficient permissions', 'listeo-data-scraper')));
        }
        ...
        if ($update_data === false) {
            wp_send_json_error(array('message' => __('Failed to check for updates. Please try again later.', 'listeo-data-scraper')));
        }
        ...
        wp_send_json_success(array(
            ...
            'message' => $update_available
                ? sprintf(__('Update available: %s', 'listeo-data-scraper'), $update_data->new_version)
                : __('You have the latest version!', 'listeo-data-scraper')
        ));
    }
    ...
    public static function force_check() {
        delete_transient('listeo_data_scraper_update_data');
        wp_clean_plugins_cache();
        ...
        if ($update_data) {
            set_transient('listeo_data_scraper_update_data', $update_data, DAY_IN_SECONDS);
            return array(
                'success' => true,
                'update_available' => version_compare(LDS_VERSION, $update_data->new_version, '<'),
                'latest_version' => $update_data->new_version
            );
        }

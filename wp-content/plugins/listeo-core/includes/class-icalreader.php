...
class ICal
{
    ...
    private /** @type {string} */ $cache_dir = null;
     ...
     */
    public function __construct($filename=false, $cache_dir = null, $cache_expire = 600)
    {
        if (!$filename) {
            return false;
        }
        
        // $this->setCache($cache_dir, $cache_expire);

        // if (is_array($filename)) {
        //     $lines = $filename;
        // } else {
        //     $lines = explode("\n", $this->getCalendar($filename));
        // }

        // Enhanced security for HTTP requests
        $request_args = array(
            'timeout'     => 30,
            'redirection' => 3,
            'user-agent'  => 'Listeo iCal Parser/1.0',
            'sslverify'   => true,
            'headers'     => array(
        ...
        $file = wp_remote_get($filename, $request_args);

        // Check for HTTP errors
        if (is_wp_error($file)) {
            throw new Exception('Failed to fetch iCal file: ' . $file->get_error_message());
        }

        $response_code = wp_remote_retrieve_response_code($file);
        if ($response_code !== 200) {
            throw new Exception('HTTP Error ' . $response_code . ' when fetching iCal file');
        }

        $contents = wp_remote_retrieve_body($file);
        ...
        if (empty($contents) || !strpos($contents, 'BEGIN:VCALENDAR')) {
            throw new Exception('Invalid iCal content received');
        }
        $lines = explode("\n", $contents);
    ...
    /**
     * Enable caching request by setting the cache directory
     * @param string $dir
     * @return \ICal
     */
    public function setCache($dir, $expire = 1800) {
    ...
    /**
     * @see https://raw.githubusercontent.com/lolalalol/phpflickr/patch-1/phpFlickr.php
     */
    protected function getCalendar($uri)
    {      
//       if ($this->cache_dir !== null) {
//         $reqhash = md5(serialize(array( 
//           'uri' => $uri,
...
//       }
    ...
    {      
        if (!is_array($lines) || stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
            return false;
        } else {

...
<?php if (in_array('layout', $segments)) : ?>
	...
	<?php else : ?>
		...
		<?php
		// Hide radius slider in pure viewport mode (not needed for bounding box searches)
		$search_mode = get_option('listeo_location_search_mode', 'radius');
		$show_radius = ($search_mode !== 'viewport') && in_array('radius', $segments);
		?>
			<?php if ($show_radius) : ?>
				<!-- Panel Dropdown-->
			...
			<?php do_action('listeo_archive_top_buttons'); ?>

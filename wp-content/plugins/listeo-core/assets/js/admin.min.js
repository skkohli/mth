...
//   );
  ...
  $('.cmb-type-listeo-calendar #listeo-calendar-outer-container').on("click", 'span.calendar-day-date', function(e) { 
     ...
     if (td.hasClass("not_active")) {
       td.removeClass("not_active");
       // Fix: use array operations instead of string.replace to avoid substring collisions
       var datesArray = current_dates.split('|').filter(function(d) { return d !== ''; });
       var idx = datesArray.indexOf(date);
       if (idx !== -1) datesArray.splice(idx, 1);
       current_dates = datesArray.length > 0 ? datesArray.join('|') + '|' : '';
     } else {
       td.addClass("not_active");
       current_dates += date + "|";
     }

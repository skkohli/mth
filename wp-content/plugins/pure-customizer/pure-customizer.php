<?php
/**
 * Plugin Name: PureCustomizer Framework
 * Plugin URI: 
 * Description: The Ultimate WordPress Customizer Framework (Fork of Kirki)
 * Author: 
 * Author URI: 
 * Version: 1.0.0
 * Text Domain: pure-customizer
...
// No need to proceed if Kirki already exists (maintaining backward compatibility).
if ( class_exists( 'Kirki' ) ) {
	return;
}

// No need to proceed if PureCustomizer already exists.
if ( class_exists( 'PureCustomizer' ) ) {
	return;
}
...
if ( ! defined( 'PURE_CUSTOMIZER_PLUGIN_FILE' ) ) {
	define( 'PURE_CUSTOMIZER_PLUGIN_FILE', __FILE__ );
}
...
require_once __DIR__ . '/inc/kirki-compatibility.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
require_once __DIR__ . '/inc/migration-helper.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
require_once __DIR__ . '/inc/debug.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude

if ( ! defined( 'KIRKI_VERSION' ) ) {
	define( 'KIRKI_VERSION', '5.0.0' );
}

if ( ! defined( 'PURE_CUSTOMIZER_VERSION' ) ) {
	define( 'PURE_CUSTOMIZER_VERSION', '1.0.0' );
}
...
if ( ! defined( 'PURE_CUSTOMIZER_PLUGIN_DIR' ) ) {
	define( 'PURE_CUSTOMIZER_PLUGIN_DIR', __DIR__ );
}

if ( ! defined( 'KIRKI_PLUGIN_URL' ) ) {
	define( 'KIRKI_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
}

if ( ! defined( 'PURE_CUSTOMIZER_PLUGIN_URL' ) ) {
	define( 'PURE_CUSTOMIZER_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
}
...
// Instantiate classes.
new Kirki();
new L10n( 'pure-customizer', __DIR__ . '/languages' );
...
/**
 * PureCustomizer Compatibility Layer
 * This ensures that existing themes using Kirki will continue to work
 */
if ( ! class_exists( 'PureCustomizer' ) ) {
	/**
	 * PureCustomizer main class - alias for Kirki compatibility
	 */
	class PureCustomizer extends Kirki {
		// Inherits all Kirki functionality
	}
}

// Create global alias functions for backward compatibility
if ( ! function_exists( 'pure_customizer' ) ) {
	/**
	 * Returns an instance of the PureCustomizer object.
	 */
	function pure_customizer() {
		return kirki();
	}
}
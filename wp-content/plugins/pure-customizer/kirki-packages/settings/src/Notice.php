...
class Notice {
		...
		>
			...
			<div class="notice-body">
				<div class="notice-content">
					<h2>
						Font Display Issues? Clear the Font Cache in PureCustomizer
					</h2>
					<p>
						<strong>New!</strong> Easily resolve font display issues by clearing the <strong>Font Cache</strong> in PureCustomizer. This one-click solution fixes problems caused by domain name changes or site migrations.
					</p>
					<p>
	...
	public function dismiss_discount_notice() {
		...
		if ( ! wp_verify_nonce( $nonce, 'Kirki_Dismiss_Discount_Notice' ) ) {
			wp_send_json_error( __( 'Invalid nonce', 'pure-customizer' ) );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( "You don't have capability to run this action", 'pure-customizer' ) );
		}
		...
		wp_send_json_success( __( 'Discount notice has been dismissed', 'pure-customizer' ) );
	}

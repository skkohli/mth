...
class TaxonomyCarousel extends Widget_Base
{
	...
	protected function render()
	{
		...
		// Handle both single and multiple taxonomy selection for backward compatibility
		if (empty($taxonomies)) {
			return;
		}

		// Convert to array if single selection (backward compatibility)
		if (!is_array($taxonomies)) {
			$taxonomies = array($taxonomies);
		}

		// Debug output (uncomment for debugging)
		// echo '<!-- Debug: Selected taxonomies: ' . print_r($taxonomies, true) . ' -->';
		// echo '<!-- Debug: All terms count before processing: ' . count($all_terms) . ' -->';

		$all_terms = array();

		// Fetch terms from all selected taxonomies
		foreach ($taxonomies as $taxonomy) {
			$query_args = array(
				'hide_empty' => false,
				'number' => 0, // Don't limit per taxonomy, we'll limit globally later
			);

			// Only add include filter if it has values
			if (!empty($settings[$taxonomy . '_include'])) {
				$query_args['include'] = $settings[$taxonomy . '_include'];
			}

			// Only add exclude filter if it has values
			if (!empty($settings[$taxonomy . '_exclude'])) {
				$query_args['exclude'] = $settings[$taxonomy . '_exclude'];
			}
			...
			if (! empty($terms) && ! is_wp_error($terms)) {
				// Add taxonomy info to each term for counting purposes
				foreach ($terms as $term) {
					$term->source_taxonomy = $taxonomy;
				}
		...
		// Debug output (uncomment for debugging)
		// echo '<!-- Debug: All terms count after fetching: ' . count($all_terms) . ' -->';
		// echo '<!-- Debug: Taxonomies processed: ' . implode(', ', $taxonomies) . ' -->';
		...
		if (!empty($settings['number']) && count($unique_terms) > $settings['number']) {
			$unique_terms = array_slice($unique_terms, 0, $settings['number']);
		}
						...
						<div class="category-box-container">
							<a href="<?php echo esc_url(get_term_link($term)); ?>" class="category-box" data-background-image="<?php if (!empty($cover)) echo $cover[0];  ?>">
								...
								<div class="category-box-content">
									...
									<?php if ($settings['show_counter'] == 'yes'): ?><span><?php $count = listeo_get_term_post_count($term->source_taxonomy, $term->term_id);
																							echo $count ?> <?php esc_html_e('listings', 'listeo_elementor'); ?></span><?php endif; ?>
								</div>

...
(function ( $ ) {
  ...
  //$("#location_search").geocomplete(geooptions);
    ...
    if ($('.main-search-input-item')[0]) {
      console.log('Listeo Viewport (Theme): Initializing Google autocomplete...');
      var input = document.getElementById('location_search');
      console.log('Listeo Viewport (Theme): Input element =', input);

      var autocomplete = new google.maps.places.Autocomplete(input, geooptions);
      console.log('Listeo Viewport (Theme): Autocomplete object created');

      autocomplete.addListener('place_changed', function() {
        console.log('Listeo Viewport (Theme): place_changed event fired!');
        var place = autocomplete.getPlace();
        console.log('Listeo Viewport (Theme): place object =', place);

        if (!place.geometry) {
          console.log('Listeo Viewport (Theme): No geometry in place object');
          return;
        }

        console.log('Listeo Viewport (Theme): place.geometry.viewport =', place.geometry.viewport);

        var form = $(this).closest('form');
        if (!form.length) form = $('#listeo_core-search-form');

        // Remove old viewport fields to avoid duplicates
        form.find('input[name^="place_viewport"]').remove();
        form.find('input[name="place_type"]').remove();

        // Capture viewport bounds from Google Places for viewport-based search
        if (place.geometry.viewport) {
            var ne = place.geometry.viewport.getNorthEast();
            var sw = place.geometry.viewport.getSouthWest();

            // Add viewport hidden fields
            form.append('<input type="hidden" name="place_viewport[ne_lat]" value="' + ne.lat() + '">');
            form.append('<input type="hidden" name="place_viewport[ne_lng]" value="' + ne.lng() + '">');
            form.append('<input type="hidden" name="place_viewport[sw_lat]" value="' + sw.lat() + '">');
            form.append('<input type="hidden" name="place_viewport[sw_lng]" value="' + sw.lng() + '">');

            // Debug logging
            console.log('Listeo Viewport (Theme): Appended fields', {
              ne_lat: ne.lat(),
              ne_lng: ne.lng(),
              sw_lat: sw.lat(),
              sw_lng: sw.lng()
          ...
          console.log('Listeo Viewport (Theme): No viewport available from Google Places API');
        }

        // Store place type for auto mode intelligent switching
        var placeType = 'unknown';
        if (place.types) {
            if (place.types.indexOf('country') !== -1) placeType = 'country';
            else if (place.types.indexOf('administrative_area_level_1') !== -1) placeType = 'region';
            else if (place.types.indexOf('locality') !== -1) placeType = 'city';
        }
        form.append('<input type="hidden" name="place_type" value="' + placeType + '">');

        console.log('Listeo Viewport (Theme): Place type =', placeType);

        // Use setTimeout to ensure DOM updates before triggering search
        setTimeout(function() {
          // Debug: Verify fields are in form
          var vpFields = form.find('input[name^="place_viewport"]');
          console.log('Listeo Viewport (Theme): Fields in form before search =', vpFields.length);
          vpFields.each(function() {
            console.log('  -', $(this).attr('name'), '=', $(this).val());
          });

          // Trigger search (this may not apply for theme map view, but keeping for consistency)
    ...
    //function mainMap() {
      ...
      if(listeo_core.centerPoint) {
        // When autofit is enabled, don't center on search location
        // Let fitBounds() handle the centering based on markers
        if(listeo_core.maps_autofit == 'on') {
            // Use default center - fitBounds will override this anyway
            var latlngStr = listeo_core.centerPoint.split(",",2);
            var lat = parseFloat(latlngStr[0]);
            var lng = parseFloat(latlngStr[1]);
            center_map = new google.maps.LatLng(lat, lng);
        } else {
            // When autofit is off, use search location as center
